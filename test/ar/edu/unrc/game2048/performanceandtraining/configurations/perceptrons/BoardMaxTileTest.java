...
public class BoardMaxTileTest {
    ...
    public BoardMaxTileTest() {
        ...
        Tile[] randomB = {
            tileContainer.getTile(0), tileContainer.getTile(4), tileContainer.getTile(0), tileContainer.getTile(11),
    ...
    public void testCalculateNormalizedPerceptronInput() {
        ...
        double actualLow = -11;
        double actualHigh = 11;
        double normalizedHigh = 1;
        double normalizedLow = -1;

        assertEquals(0d, board.translateToPerceptronInput(0), 0.0d);
        assertEquals(0d, board.translateToPerceptronInput(1), 0.0d);
        assertEquals(0d, board.translateToPerceptronInput(2), 0.0d);
        assertEquals(0d, board.translateToPerceptronInput(3), 0.0d);

        assertEquals(0d, board.translateToPerceptronInput(4), 0.0d);
        assertEquals(0d, board.translateToPerceptronInput(5), 0.0d);

        double normalization = ((2d - actualLow) / (actualHigh - actualLow))
                * (normalizedHigh - normalizedLow)
                + normalizedLow;
        assertEquals(normalization, board.translateToPerceptronInput(6), 0.0d);
        assertEquals(0d, board.translateToPerceptronInput(7), 0.0d);

        assertEquals(0d, board.translateToPerceptronInput(8), 0.0d);
        assertEquals(0d, board.translateToPerceptronInput(9), 0.0d);
        assertEquals(0d, board.translateToPerceptronInput(10), 0.0d);
        assertEquals(0d, board.translateToPerceptronInput(11), 0.0d);

        normalization = ((4d - actualLow) / (actualHigh - actualLow))
                * (normalizedHigh - normalizedLow)
                + normalizedLow;
        assertEquals(normalization, board.translateToPerceptronInput(12), 0.0d);
        assertEquals(0d, board.translateToPerceptronInput(13), 0.0d);
        assertEquals(0d, board.translateToPerceptronInput(14), 0.0d);
        assertEquals(0d, board.translateToPerceptronInput(15), 0.0d);

        board = new GameBoard<>(game, tileContainer);
        board.setTiles(randomBoard1);
        board.updateInternalState(true); //dentro de este metodo se utiliza el "calculateNormalizedPerceptronInput"

        normalization = ((0d - actualLow) / (actualHigh - actualLow))
                * (normalizedHigh - normalizedLow)
                + normalizedLow;
        ...
        normalization = ((2d - actualLow) / (actualHigh - actualLow))
                * (normalizedHigh - normalizedLow)
                + normalizedLow;
        ...
        normalization = ((6d - actualLow) / (actualHigh - actualLow))
                * (normalizedHigh - normalizedLow)
                + normalizedLow;
        ...
        normalization = ((5d - actualLow) / (actualHigh - actualLow))
                * (normalizedHigh - normalizedLow)
                + normalizedLow;
        assertEquals(normalization, board.translateToPerceptronInput(10), 0.0d);
        normalization = ((1d - actualLow) / (actualHigh - actualLow))
                * (normalizedHigh - normalizedLow)
                + normalizedLow;
        assertEquals(normalization, board.translateToPerceptronInput(11), 0.0d);

        normalization = ((11d - actualLow) / (actualHigh - actualLow))
                * (normalizedHigh - normalizedLow)
                + normalizedLow;
        assertEquals(normalization, board.translateToPerceptronInput(12), 0.0d);
        normalization = ((4d - actualLow) / (actualHigh - actualLow))
        ...
        normalization = ((7d - actualLow) / (actualHigh - actualLow))
                * (normalizedHigh - normalizedLow)
                + normalizedLow;
    ...
    public void testTranslateThisFinalStateToPerceptronOutput() {
        ...
        assertEquals(Math.pow(2, 4), board.getMaxTileNumberValue(), 0.0d);

        board = new GameBoard<>(game, tileContainer);
        board.setTiles(randomBoard1);
        board.updateInternalState(true);

        assertEquals(Math.pow(2, 11), board.getMaxTileNumberValue(), 0.0d);
    }
